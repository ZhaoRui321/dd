...
void frmNum::InitProperty()
{
    ...
    ui->btnClear->setProperty("btnOther",true);
}
...
void frmNum::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
    ...
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
        ...
        if(pos.x() < deskRect.width()/2 ){
                this->setGeometry(deskRect.width()-160-this->width(), deskRect.height()-this->height(), this->width(), this->height());
        }else{
                this->setGeometry(240, deskRect.height()-this->height(), this->width(), this->height());
        }

        if(nowWidget->property("special").toBool())
        {
            this->setGeometry(deskRect.width()-160-this->width(), deskRect.height()-this->height(), this->width(), this->height());
        }
    }
...
{
    QStringList qss;
    qss.append(QString("QWidget#frmNum{background-color: rgb(216,216,216);}"));
               //.arg(topColor).arg(bottomColor));
    qss.append("QPushButton{background-color: white;padding:5px;border-radius:3px;}");
//    qss.append(QString("QPushButton:hover{background:qlineargradient(spread:pad,x1:0,y1:0,x2:0,y2:1,stop:0 %1,stop:1 %2);}")
//               .arg(topColor).arg(bottomColor));
    qss.append(QString("QLabel,QPushButton{font-size:25px;color:black;}"));
