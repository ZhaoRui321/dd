...
///
    ...
    class roots{
    public:
        static polyRoots(const double *factor
                         ,size_t factorLength
                         ,std::vector<double>& out_factor){
            //int i;
            //double a[6] = { -1, 0, 0, 0, 0, 1 };
            //double z[10];
            out_factor.resize((factorLength-1)* 2);
            gsl_poly_complex_workspace * w = gsl_poly_complex_workspace_alloc (factorLength);
            gsl_poly_complex_solve (factor, factorLength, w, out_factor.data());
